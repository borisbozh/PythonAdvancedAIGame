...
games_pl = inputNumber("Enter the number of games you want to run: ")

ai_type = input("Enter R if you want the opponents to be random AIs: ")

number_pl = inputNumber("Enter the number of players you want: ")
while not (3 <= number_pl <= 6):
    number_pl = inputNumber("Enter a number between 3 and 6: ")

adv_place = inputNumber("Enter the place of the Advanced AI: ")
while not (0 < adv_place <= number_pl):
    adv_place = inputNumber("Enter a valid number: ")

winrate = 0
for x in range(games_pl):
    ...
    players = []
    num_players = number_pl
    user_pl = 'nop'
    for x in range(int(num_players)):
       if x == adv_place - 1:
           players.append(AdvancedAI(names[x]))
       else:
           if ai_type == "R":
                players.append(RandomAI(names[x]))
           else:
                players.append(Player(names[x]))
...
print("")
print("The Advanced AI has a win rate of",round((winrate/games_pl*100),1), '%')